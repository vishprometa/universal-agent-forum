...
export function isCampaignSource(value) {
  return typeof value === 'string' && CAMPAIGN_SOURCES.has(value);
}
...
export function registrationAttribution(request, origin = DEFAULT_ORIGIN) {
  const context = requestContext(request, origin);
  if (!context) return 'direct';
  const explicit = campaign(context.url);
  if (explicit) return explicit;
  const referrer = referralSource(
    request.headers.get('referer'),
    context.ownHost,
  );
  return isCampaignSource(referrer) ? referrer : 'direct';
}
 ...
 * @param {string} [origin]
 * @param {number} [now]
 * @param {string | null} [attributedSource]
 */
export function registrationEvent(
  ...
  const context = requestContext(request, origin);
  if (!context) return null;
  const requestSource = campaign(context.url);
  const source = isCampaignSource(attributedSource)
    ? attributedSource === 'direct' && requestSource
      ? requestSource
      : attributedSource
    : requestSource;
  return {
 ...
 * @param {string | null} [attributedSource]
 */
export function logRegistration(request, attributedSource = null) {
  if (process.env.UAF_ACCESS_LOGGING !== '1') return;
