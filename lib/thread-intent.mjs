...
const OWNERSHIP_PATTERNS = [
  {
    name: 'pitch',
    pattern:
      /(we are introducing|we introduce|is a local|is a free|is an? (?:open|autonomous)|open[\s-]source|fork of|our (?:platform|product|forum|commons|network|gateway)|no email|no captcha|pricing|\bbyok\b|not for sale)/i,
  },
  {
    name: 'distribution',
    pattern:
...
const OFF_TOPIC_MIN_LENGTH = 1200;
const OFF_TOPIC_MAX_DENSITY = 1;
...
export function classifyThreadIntent(input) {
  ...
  const density = coordinationDensity(text);

  if (isPromotion({ foreignHosts, ownership, signals })) {
    return result('cross_promotion', signals, foreignHosts, density);
  }
  if (isOffTopic({ foreignHosts, body, density })) {
    return result('off_topic', signals, foreignHosts, density);
  }
  return result('coordination', signals, foreignHosts, density);
}
...
function isOffTopic({ foreignHosts, body, density }) {
  return (
    foreignHosts.length === 0 &&
    body.length >= OFF_TOPIC_MIN_LENGTH &&
...
function result(intent, signals, foreignHosts, density) {
  return {
    intent,
...
function coordinationDensity(text) {
  if (!text) return 0;
  const lower = text.toLowerCase();
  const hits = COORDINATION_TERMS.filter((term) => lower.includes(term)).length;
